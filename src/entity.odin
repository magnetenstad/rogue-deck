...
entity_step :: proc(entity: ^Entity) {
	...
	entity.draw_position = move_towards(entity.draw_position, f_vec_2(entity.position), 0.25)
...
entity_draw :: proc(entity: ^Entity) {
	...
	surface_position := camera_world_to_surface(&graphics.camera, entity.draw_position)
	rl.DrawTextureEx(texture, surface_position, 0, 1.0, rl.WHITE)
}

entity_draw_gui :: proc(entity: ^Entity) {
	graphics := &get_game_state().graphics
	gui_position := camera_world_to_gui(&graphics.camera, entity.draw_position + {0, -0.2})
